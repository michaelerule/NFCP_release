function [varargout] = gp(hyp, inf, mean, cov, lik, x, y, xs, ys)
    ...
    Copyright (c) by Carl Edward Rasmussen and Hannes Nickisch, 2014-01-06.
    File automatically generated using noweb.

    Modifications: M.Rule, 2017: reformatted tabs, spaces, comments;
    error-reporting improved: stack traces recorded properly
    %}
    ...
    if ischar(mean) || isa(mean, 'function_handle'), mean = {mean}; end 
    % make cell
    if isempty(cov), error('Covariance function cannot be empty'); end 
    % no default
    ...
    %only one possible class of inference algorithms
    ...
    try
        ...
        % issue a warning if a classification likelihood is used in conjunction
        % with labels different from +1 and -1
        ...
        if nargin>7% compute marginal likelihood and its derivatives only if needed
            if isstruct(y)
                ...
                assert(size(post.alpha,1)<=size(x,1));
            end
        else
            if nargout==1
    ...
    % 2017 11 07 M.Rule : modified these error routines
        ...
        if nargin > 7, 
            ...
            msg = sprintf('Inference method failed %s .. aborting',e.message);
            ...
            fprintf(2,'Message:\n%s\n',e.message);
            fprintf(2,'%s\n',getReport(e,'extended'));
            msg = sprintf('Inference method failed %s .. aborting for safety',e.message);
    ...
    if nargin==7 
        ...
        if issparse(alpha) 
            ...
            %assert(all(size(nz)==[size(alpha,1)
        end 
        ...
        while nact<ns
            ...
            Fmu = repmat(ms,1,N) + Ks'*full(alpha(nz,:)); 
            % predictive means
            fmu(id) = sum(Fmu,2)/N; 
            if Lchol 
