function model = initializeModel(model,varargin)
    %{
    ...
    if isfield(model,'linearRates') && isfield(model,'linearTransitions'),
        if model.verbosity>0 && isfield(model,'description'),
            fprintf(2,['Both description and (linearRates,linearTransitions) '...
    ...
    elseif isfield(model,'description'),
        if model.verbosity>0,
            if isfield(model,'linearRates'),
                fprintf(2,['description and linearRates both defined, '...
                'but missing linearTransitions. Using description matrix\n']);
            end
            if isfield(model,'linearRates'),
                fprintf(2,['description and linearTransitions both defined, '...
    ...
    if ~(all(model.linearTransitions(1:end,1)==excitatory_reaction) ||...
         all(model.linearTransitions(1:end,1)==reverse_excitatory_reaction)),
        fprintf(2,['Error when parsing model description: first reaction is not '...
            'an excitatory reaction\n'...
            'For now,  all models must have at least two states,  '...
            'Quiescent (Q) and Active (A).\nFor neurons,  the quiescent state '...
            'should correspond to not firing,  and the active\nstate should '...
            'correspond to firing.  The first state must be Q and the '...
            'second state\nmust be A. The first reaction must represent '...
            'excitation of a quiescent neuron into\nthe active state. '...
            '  The reason for this is that the excito-excitatory interactions\n'...
            'are approximated by an effective excitation rate depending on the '...
            'number of active\nneurons.     This approximation is combined with '...
            'time-homogenous linear reactions,\nand the relevant code assumes '...
            'that the first population is Q, the second is A, and\nthat the first '...
    ...
    if ~isfield(model,'names'),
        ...
        if size(model.names,2)~=model.nstates,
            warning(sprintf('There were %d entries in model.names, but %d states defined',...
