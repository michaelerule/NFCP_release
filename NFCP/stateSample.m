function [ini,xydata,rates,simulatedM] = sampleSates(model,varargin)
    %{
    ...
    opt = applyOptions({...
        'doplot'      ,true ,'';...
        'Nsample'     ,500  ,'Number of sample time-points to simulate';...
        'Nburn'       ,200  ,'Initial number of burn-in samples to reach steady-state';...
    ...
    for i=1:opt.Nsample
        ...
        if opt.doplot && mod(i,opt.skipsim)==0
            ...
            NFCP_plotting.showField(trueRGB,...
...
function M = PoissonEvents(model,M)
    %{
...
function [spikes,rate] = sampleSpikes(model,M,opt)
    %{ 
    ...
    spikes = max(min([x y]./(model.n*opt.oversample) + ...
