function [llsum,infstate,margvar,infe,sM,sP,sC] = stateInfer(ini,model,xydata,true_states,varargin)
    %{
    ...
    for isamp=1:Nsample
        ...
        if opt.doplot,
            ...
            % Plot progress
            [inf_means,inf_errs] = NFCP_plotting.do_plot(model,isamp,M,C,...
...
function [model,opt] = verifyArguments(ini,model,xydata,true_states,opt)
    %{
...
function opt = getUpdateModel(model,opt)
    %{
