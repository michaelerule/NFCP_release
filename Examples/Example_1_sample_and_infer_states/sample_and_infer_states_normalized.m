...
rescaledM = {};
for i=1:Nsample,
    rescaledM{i} = simulatedM{i}./density;
end
rescaledini = ini./density;
...
infermodel.thr            = 0.0;
infermodel.update         = 'Laplace'; 
infermodel.cutoff         = false

% These parameters change when working with normalized coordinates
infermodel.gain           = model.gain.*density;
infermodel.ss_rescale     = 1./density;
infermodel.minrate        = model.minrate./density
infermodel.rAA            = 1.40;  % Excito-Excitatory interaction strength
infermodel.thr            = 8e-3./density;  % Nonzero threshold for depolarization
