...
%{
% Incorrect parameters, what happens?
...
model.dolikelihood  = false; % Disable likelihood for now, for speed
...
fstates = fft(states,[],2);
pstates = reshape(mean(abs(fstates).^2,[1]),Nsample,1);
...
states  = reshape(states,N*N,3,Nsample);
K = sum(goodregions);
q = reshape(states(goodregions,1,:),K*Nsample,1);
a = reshape(states(goodregions,2,:),K*Nsample,1);
r = reshape(states(goodregions,3,:),K*Nsample,1);
subplot(212);
cla; hold all;
